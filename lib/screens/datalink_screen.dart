...
void _zipIsolateEntry(List<dynamic> args) async {
  ...
  try {
    ...
    try {
      ...
      // Falls Scan fehlschlägt, weiter machen
    }
...
class _DataLinkScreenState extends State<DataLinkScreen> with WidgetsBindingObserver {
  ...
  Future<void> _ensureServiceStarted({bool showNotification = false}) async {
    if (!_serviceStarted && Platform.isAndroid) {
      ...
      // Nur Notification updaten wenn explizit gewünscht
  ...
  void _startHeartbeatLoop() {
    _heartbeatTimer = Timer.periodic(const Duration(seconds: 2), (t) {
      _sendHeartbeat();
      _syncTransfers();
      _monitorSenderTasks();
    });
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (mounted) {
          setState(() {
            _isConnected = true;
            _activePeers = json.decode(response.body)['active_peers'];
          });
        }
      }
    } catch (e) {
...
Future<void> _offerFile(String filePath, String targetId) async {
  ...
  Future<void> _pickAndSendFolder() async {
    ...
    // Service starten aber KEINE Notification zeigen
      ...
      await for (final message in receivePort) {
        if (message is ZipProgress) {
          ...
          if (message.resultPath != null) {
            ...
            // ✅ NUR HIER Notification zeigen (Zip fertig)
            ...
            // Nur UI updaten, keine Notification
            setState(() {
  ...
  Future<void> _uploadToRelay(String path, String transferId) async {
    ...
    // Service starten aber KEINE Notification während Upload
    await _ensureServiceStarted(showNotification: false);
      ...
      final request = ProgressMultipartRequest(
        ...
        onProgress: (bytes, total) {
          if (mounted) {
            ...
            // Keine Notifications während Upload
          }
        },
      );
      ...
      if (response.statusCode == 200) {
        ...
        for (var t in list) {
          ...
          if (status == "OFFERED") {
            ...
            // ✅ Notification bei EMPFANG eines neuen Transfers
