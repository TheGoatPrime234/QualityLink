...
class _NetworkStorageScreenState extends State<NetworkStorageScreen> {
  ...
  void _onConnectionChanged(bool isConnected) {
    if (mounted) {
      ...
      if (isConnected) {
        _showSnack("Connected to server");
        _loadDevices();
      } else {
  ...
  Future<void> _loadDevices() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _loading = false;
          if (_devices.isEmpty) _isConnected = false;
        });
      }
    }
  }
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: _isConnected 
                    ? [BoxShadow(color: const Color(0xFF00FF41).withValues(alpha: 0.5), blurRadius: 8, spreadRadius: 2)]
                    : null,
              ),
            ),
          ],
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            _isConnected ? "No devices with storage found" : "Not connected to server",
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border(
                left: BorderSide(
                  color: (isOnline && hasFileServer) ? const Color(0xFF00FF41) : Colors.grey,
                  width: 4,
                ),
              ),
            ...
            child: Row(
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          if (hasFileServer) ...[
                            Icon(Icons.folder_shared, size: 12, color: const Color(0xFF00FF41).withValues(alpha: 0.7)),
                            const SizedBox(width: 4),
                            Text(
                              "${(device['available_paths'] as List?)?.length ?? 0} storage location(s)",
                              style: TextStyle(color: const Color(0xFF00FF41).withValues(alpha: 0.7), fontSize: 11),
                            ),
                          ] else ...[
                            Icon(Icons.folder_off, size: 12, color: Colors.grey.withValues(alpha: 0.5)),
                            const SizedBox(width: 4),
                            Text(
                ...
                Icon(
                  (isOnline && hasFileServer) ? Icons.chevron_right : Icons.cloud_off,
                  color: Colors.grey,
                  size: 24,
                ),
              ],
...
class _FileBrowserScreenState extends State<FileBrowserScreen> {
  ...
  Future<void> _loadPath(String path) async {
    ...
    try {
      ...
      if (ip == null || port == null) throw Exception("Device IP or port not available");
      ...
      if (mounted) {
        _showSnack("Failed to load directory", isError: true);
...
void _openItem(Map<String, dynamic> file) {
  ...
  Future<void> _performSearch(String query) async {
    ...
    try {
      ...
      _showSnack("Search failed", isError: true);
      setState(() => _loading = false);
    }
  }
  ...
  Future<void> _uploadFile() async {
    ...
    try {
      await _datalink.sendFile(file, [targetId], destinationPath: _currentPath);
    } catch (e) {
          ...
          return AlertDialog(
            ...
            title: Column(
              ...
              children: [
                ...
                Text(targetDevice['name'], style: const TextStyle(color: Color(0xFF00FF41), fontSize: 12)),
              ],
            ),
            content: SizedBox(
                        ...
                        if (currentPath != "Root")
                          IconButton(
                            ...
                            onPressed: () {
                              ...
                              if (parent == currentPath) loadTargetFolders(setState, "Root");
                              else loadTargetFolders(setState, parent);
                            },
    ...
    await showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) {
          ...
          return AlertDialog(
            backgroundColor: const Color(0xFF1A1A1A),
            title: Text(selectDestination ? "Select Target & Folder" : "Send to Device", style: const TextStyle(color: Colors.white)),
            content: SizedBox(
              width: double.maxFinite,
              child: loading 
  ...
  Future<void> _triggerRemoteSend(
    ...
    for (var path in paths) {
      try {
        ...
        if (destinationPath != null) body['destination_path'] = destinationPath;
    ...
    if (successCount > 0) _showSnack("✅ Started sending $successCount files");
    else _showSnack("❌ Failed to initiate transfer", isError: true);
  }
            ...
            PopupMenuButton<String>(
              ...
              itemBuilder: (context) => [
                ...
                const PopupMenuItem(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.drive_file_move, color: Color(0xFF00E5FF)),
                      SizedBox(width: 12),
                      Text("Send to Folder..."),
                    ],
                  ),
